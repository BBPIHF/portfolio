...
const Experience = () => {
  ...
  return (
    <div id="experience" className="section py-[50px] bg-primary">
      <h1 className="text-white lg:text-7xl md:text-5xl text-5xl heading-line flex items-center font-bold pb-10">
          ...
          {btn.map((data, index) => {
            ...
            return (
              <button
                ...
                className={`bg-primary p-6 text-tertiary ${
                  status ? "status" : ""
                } text-left btn text-2xl w-full font-mono`}
              >
                {name}
              </button>
            );
            ...
            {responsibilities.map((data, index) => {
              return (
                <div key={index} className="flex items-center gap-8">
                  <p className="pt-4 pl-10 text-[16px] relative bullet text-slate-400 ">
                    {data.name}
                  </p>
                  {data.link !== undefined && (
                    <a
                      href={data.link}
                      target="_blank"
                      className="mt-5 italic text-secondary py-1 px-2 bg-slate-600 hover:bg-slate-700 text-xl rounded-lg"
                    >
                      link here
                    </a>
                  )}
